...
def main():
    ...
    print("  Fetching neuromaps receptor maps (one-time, internet/OSF)…")
    ...
    for system, src, desc, res in RECEPTOR_MAPS:
        try:
            ...
            print(f"    {system}: fetch failed ({str(e)[:80]}); skipping")
    if not rec_data:
    ...
    for pi, gb in enumerate(parcels):
        ...
        cent[pi] = (aff @ m)[:3]
    if n_par > 12000:
        print(f" {n_par} parcels is large; consider a bigger --block for speed/RAM.")
    D = cdist(cent, cent)   
    ...
    if args.n_perm > 0:
        ...
        for system, vec in rec_parcel.items():
            ...
            rec_surrogates[system] = np.asarray(gen(n=args.n_perm))  
