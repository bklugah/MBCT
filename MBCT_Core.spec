...
"""
MBCT_Core.spec — MBCT (core edition): the build described in the manuscript.

Tabs: Home, Analysis, Results, Help.  No Brain Viewer / Connectivity / Utilities.

Build on the TARGET OS (no cross-compiling):
    pyinstaller MBCT_Core.spec --noconfirm
Output: dist/MBCT-Core/
"""
...
print("=" * 62)
print("Building MBCT — CORE edition (Analysis + Results)")
print("=" * 62)

datas = []
# read-only data the core edition needs
add_dir(datas, ROOT, 'assets')
add_dir(datas, ROOT, 'mni_templates')
add_dir(datas, ROOT, 'cbig_network_correspondence_data')
# white-matter atlases: the Results crosshair reports JHU regions/tracts
add_dir(datas, ROOT, 'white_matter_atlases')
add_json_bundles(datas, ROOT, ['neurosynth_decoding.json',
                               'neurotransmitter_mapping.json',
                               'transcriptomics_mapping.json'])

# The correspondence engine needs the full heavy stack (VTK, regfusion, …).
pdatas, binaries, hiddenimports = collect_packages(DYNAMIC_PKGS_FULL)
datas += pdatas
for pkg in ('regfusion', 'pymare', 'nimare', 'cbig_network_correspondence'):
    collect_pkg_tree(datas, pkg)
hiddenimports += SYMPY_HIDDEN + VTK_HIDDEN
...
a = Analysis(
    ...
    binaries=binaries,
    datas=datas,
    hiddenimports=hiddenimports + ['nct_desktop_app_IMPROVED'],
...
coll = COLLECT(exe, a.binaries, a.zipfiles, a.datas,
               strip=False, upx=False, upx_exclude=[], name='MBCT-Core')

if sys.platform == 'darwin':
    app = BUNDLE(coll, name='MBCT-Core.app', icon=icon_for(ROOT),
                 bundle_identifier='com.bklugah.mbct.core')
